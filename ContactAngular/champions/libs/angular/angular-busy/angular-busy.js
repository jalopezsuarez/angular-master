(function(window, angular, undefined)
{
	...
	angular.module('ngBusy.interceptor', []).provider('busyInterceptor', function()
	{
	...
	// complete: <button busy="Loading..." busy-when-url="string"
	// busy-when-name="string" busy-add-classes="string"
	// busy-remove-classes="string" busy-disabled="bool"
	// not-busy-when-url="string" not-busy-when-name="string"
	// not-busy-add-classes="string" not-busy-remove-classes="string"
	// not-busy-disabled="bool" />

	angular.module('ngBusy.busy', []).directive('busy', [ '$parse', '$timeout', function($parse, $timeout)
	{
		return {
			restrict : 'A',
			tranclude : true,
			...
			link : function(scope, element, attrs)
			{
				...
				{
					var key;
					if (scope[(key = begin ? 'busyWhenName' : 'notBusyWhenName')])
						return !!config.name && !!config.name.match(scope[key]);
					else if (scope[(key = begin ? 'busyWhenUrl' : 'notBusyWhenUrl')])
						return !!config.url && !!config.url.match(scope[key]);
					else
						return begin === true || config.remaining <= 0;
				};
				...
				scope.$on('busy.begin', function(evt, config)
				{
					if (!scope.busy && scope.isBusyFor(config, true))
					{
						...
						if (scope.busyDisabled)
							$timeout(function()
							{
								element.attr('disabled', true);
							});
						var msgElement = scope.busyMessageElement ? scope.busyMessageElement.clone() : null;
						if (msgElement || scope.busyMessage)
							element.html('').append(msgElement || scope.busyMessage);
	...
	} ]).directive('busyMessage', function()
	{
		return {
			...
			{
				// we're basically going to transclude the content, strip it,
				// and set the busy message as the resulting transcluded HTML
				// via the controller setBusyMessageElement function
